...
from recurrent.mylearning import createRnnLibrary, offlineLearning
...
from recurrent.util import tree_stack
...
def rnnStep(
    ...
    temp = w_rec @ torch.cat((x, a, torch.tensor([1.0])))
    return (1 - config.alpha) * a + config.alpha * config.activationFn(temp)
